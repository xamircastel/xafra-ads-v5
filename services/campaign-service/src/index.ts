...
// Main routes
app.use('/campaigns', campaignRoutes);      // Campaign CRUD operations
app.use('/analytics', analyticsRoutes);     // Campaign analytics and insights
app.use('/performance', performanceRoutes); // Performance optimization
app.use('/reports', reportsRoutes);         // Campaign reporting

// Default info endpoint
app.get('/', (req, res) => {
  res.json({
    service: 'Xafra Ads Campaign Service',
    version: '5.0.0',
    status: 'running',
...
const startServer = async (): Promise<void> => {
  try {
    ...
    const gracefulShutdown = async (signal: string) => {
      ...
      server.close(async () => {
        try {
          ...
          logger.info('✅ Campaign Service shut down gracefully');
          process.exit(0);
        } catch (error) {
    ...
    loggers.error('Failed to start Campaign Service', error as Error);
    process.exit(1);
  }
};
